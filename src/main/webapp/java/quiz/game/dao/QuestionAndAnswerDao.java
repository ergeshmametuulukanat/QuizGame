...
public class QuestionAndAnswerDao extends DBConnector {
    ...
    public ArrayList<QuestionAndAnswer> getAllQuestionAndAnswer() {
        ...
        try (Connection conn = connect()) {
            ...
            while (rs.next()) {
                questionAndAnswer = new QuestionAndAnswer(rs.getInt("question_and_answer_id"),
                        rs.getInt("quiz_id"),
    ...
    public QuestionAndAnswer getQuestionAndAnswer(int questionAndAnswerId) {
        ...
        String SQL = "SELECT * FROM question_and_answer where question_and_answer_id = ?";
        try (Connection conn = connect()) {
            PreparedStatement stmt = conn.prepareStatement(SQL);
    ...
    public boolean deleteQuestionAndAnswer(int questionAndAnswerId) {
        String SQL = "delete from question_and_answer where question_and_answer_id = ?";
        try (Connection conn = connect();
             PreparedStatement stmt = conn.prepareStatement(SQL)
        ) {
            stmt.setInt(1, questionAndAnswerId);
